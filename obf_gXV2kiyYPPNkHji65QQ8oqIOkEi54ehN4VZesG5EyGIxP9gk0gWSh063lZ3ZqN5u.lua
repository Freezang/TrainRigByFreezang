...
BackFrame.Size = UDim2.new(0.7, 0, 0.6, 0)
...
-- Modifier les paramètres de disposition des cellules
Grid.CellSize = UDim2.new(0.12, 0, 0.18, 0) -- Taille des cellules réduite
Grid.CellPadding = UDim2.new(0.005, 0, -0.01, 0) -- Espacement réduit entre les cellules
Grid.SortOrder = Enum.SortOrder.LayoutOrder
Grid.Parent = Frame

-- Créer un cadre en bas à droite
local CreditFrame = Instance.new("Frame")
CreditFrame.Size = UDim2.new(0.22, 0, 0.06, 0) -- Taille légèrement plus grande
CreditFrame.AnchorPoint = Vector2.new(1, 1) -- Ancrage en bas à droite
CreditFrame.Position = UDim2.new(1, -4, 1, 25) -- Position plus en bas avec une marge
CreditFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30) -- Couleur de fond gris foncé
CreditFrame.BackgroundTransparency = 0 -- Opacité du cadre
CreditFrame.BorderSizePixel = 0 -- Pas de bordure
CreditFrame.Parent = BackFrame -- Ajouter au cadre principal

-- Ajouter un texte au cadre
local CreditText = Instance.new("TextLabel")
CreditText.Size = UDim2.new(1, 0, 1, 0) -- Remplit tout le cadre
CreditText.BackgroundTransparency = 1 -- Pas de fond pour le texte
CreditText.Text = "| Remake by Freez4ng" -- Texte à afficher
CreditText.TextScaled = true -- Texte ajusté automatiquement
CreditText.TextColor3 = Color3.new(1, 1, 1) -- Texte blanc
CreditText.Font = Enum.Font.SourceSansBold -- Police en gras
CreditText.Parent = CreditFrame -- Ajouter au cadre
...
-- Fonction pour vérifier si la souris est en dehors du cadre BackFrame
local function isOutsideFrame(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        local mousePos = UserInputService:GetMouseLocation()
        local framePos = BackFrame.AbsolutePosition
        local frameSize = BackFrame.AbsoluteSize

        -- Vérifie si le clic est en dehors des limites de BackFrame
        if mousePos.X < framePos.X or mousePos.X > framePos.X + frameSize.X
            or mousePos.Y < framePos.Y or mousePos.Y > framePos.Y + frameSize.Y then
            return true
        end
    end
    return false
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and ScreenGui.Enabled then
        if isOutsideFrame(input) then
            ScreenGui.Enabled = false
        end
    end
end)
